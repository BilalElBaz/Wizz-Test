...
describe('POST /api/games/search', () => {
  it('should respond with json containing a list that includes name of the game we just created', (done) => {
    request(app)
  ...
  it('should respond with json containing empty list since name does not match the game we just created', (done) => {
    request(app)
      .post('/api/games/search')
      .send({ name: 'banana' })
      .set('Accept', 'application/json')
  ...
  it('should respond with json containing a list that includes the platform of game we just created', (done) => {
    request(app)
      .post('/api/games/search')
      .send({ platform: 'ios' })
      .set('Accept', 'application/json')
  ...
  it('should respond with json containing a list that includes name and platform of game we just created', (done) => {
    request(app)
      .post('/api/games/search')
...
describe('POST /api/games/populate', () => {
  it('should respond with created', (done) => {
    request(app)
      ...
      .expect(201)
      .end((populateErr) => {
        if (populateErr) return done(populateErr);
        ...
        request(app)
          ...
          .end((err, result) => {
            if (err) return done(err);
            assert.notStrictEqual(result.body.length, 0);
            done();
          });
      });
  });
